/**
 * @file adt/PriorityQueue.h - the Queue ADT
 ...
 *
 * The PriorityQueue Abstract Data Type is a linked list of same-datatype elements where insertions
 * happen at one end and deletions happen at the other.
 * The PriorityQueue can be enlarged by enqueueing an element of type T to it, but when an element is
 * enqueued it is inserted in order. (Note that T must have an accessible default constructor.) It can 
 * be shrunk by the dequeue method. Only the element about to be dequeued can be accessed. It is accessed
 ...
 */
template <typename T>
std::ostream& operator<<(std::ostream& out, const PriorityQueue<T>& prioQueue) {
    return prioQueue.print(out);
}
