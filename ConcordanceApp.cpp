/**
  *@file ConcordanceApp.cpp - implementation of ConcordanceApp class
  *@author Nassuel Valera Cuevas
  *@see "Seattle University CPSC2430 Spring 2018"
  */

#include <iostream>
#include "ConcordanceApp.h"
#include <string>

void ConcordanceApp::run() {
	...
	while (1) {
		...
		if (com == Command::QUIT) {
			...
			const List<ConcordanceBST::WordRef> &words = concordance.lookupWord(wordToLook);
...
ConcordanceApp::Command ConcordanceApp::getCommand(std::string &name) {
...
std::string ConcordanceApp::getFilename(std::string prompt) {	
	prompt = prompt.substr(prompt.find(" ") +1, prompt.size());	
	prompt[0] = toupper(prompt[0]);
	return prompt;
}

void ConcordanceApp::displayWords(const List<Concordance::WordRef> &words) {
...
void ConcordanceApp::displayPassage(const Concordance::WordRef &ref) {
...
int ConcordanceApp::getSelection(std::string prompt, int maxSelection) {
	// Absolutely no idea what this method is supposed to do
	return 0;
}

void ConcordanceApp::lookup(std::string word) {
